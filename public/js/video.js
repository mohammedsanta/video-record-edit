...
function addTrim() {
    ...
    createEndLabel.textContent = 'Durations (seconds)';
...
const screenshot = async () => {
    ...
    const url = `http://127.0.0.1:3000/video/get/screenshot?video=${selectElement.value}&time=${videoCurrentTime}`;
    ...
    downloadLink.href = blobUrl; // Blob URL for the screenshot
    downloadLink.download = `screenshot-${videoCurrentTime}.png`; // Name the file
    document.body.appendChild(downloadLink);
    downloadLink.click();
    document.body.removeChild(downloadLink);

    // Optionally, revoke the blob URL after download to free up resources
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const session  = fullUrl.split('/').pop()

    // Function to fetch data and populate the select input
    async function fetchAndPopulateSelect() {
        try {
            const response = await fetch(`http://127.0.0.1:3000/session/videos/${session}`); // Replace with your API URL
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
    ...
    async function fetchVideos(apiUrl) {
        try {
            const response = await fetch(`http://127.0.0.1:3000/videos/${apiUrl}`);
            ...
            option.value = data.video._id; // Set the value to the item's id
            option.textContent = data.video.title; // Display the item's name
            selectElement.onchange = function() {
                changeVideo(selectElement.value); // Call changeVideo with the selected value
            };
            selectElement.appendChild(option);
            changeVideo(selectElement.firstElementChild.value);
    ...
    // Call the function to fetch data and populate the select
...
const noVideos = () => {
    ...
    const buttonRedirect = noVideoModal.firstElementChild.getElementsByTagName('button');
    buttonRedirect[0].addEventListener('click',() => {
        console.log(buttonRedirect[0])
        window.location.replace('http://127.0.0.1:3000/');
    })


    noVideoModal.style.display = "block";
...
function changeVideo(video) {
    ...
    const videoPath = `http://127.0.0.1:3000/video/${video}`

    console.log(videoSource)
    videoSource[0].src = videoPath;
    videoPlayer[0].load(); // Load the new video

    const uploadFormVideo = document.getElementById('uploadFormVideo')
    uploadFormVideo.value = video

    console.log('Selected video ID:', uploadForm);
    // Add your logic to handle the video change, e.g., load the video
}